...
class PINNs(models.Model):
    ...
    def net_f(self, cp):
        ...
        f1 = - u_t - u * u_x - v * u_y - p_x + (1/100) * (u_xx + u_yy)
        f2 = - v_t - u * v_x - v * v_y - p_y + (1/100) * (v_xx + v_yy)
        f3 = u_x + v_y
    ...
    def train_step(self, ic, bc, cp):
        with tf.GradientTape() as tape:
            ...
            loss_bc = tf.reduce_mean(tf.square(bc[:, 3:] - u_p_bc[:, :-1]))
            loss_ic = tf.reduce_mean(tf.square(ic[:, 3:] - u_p_ic[:, :-1]))
