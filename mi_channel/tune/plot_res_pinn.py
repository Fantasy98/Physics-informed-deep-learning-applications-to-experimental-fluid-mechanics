...
argparser.add_argument("--nn",default=20,type=int, help="Number of neurons")
argparser.add_argument("--nl",default=4,type=int, help="Number of layers")
argparser.add_argument("--sw",default=1,type=float, help="Weight of supervised learning")
argparser.add_argument("--uw",default=10,type=float, help="Weight of unsupervised learning")
...
pred2 = np.load(f'../results/res_pinn_t{args.t}_s{args.s}_c{args.c}_nn{args.nn}_nl{args.nl}_{args.sw}sw_{args.uw}uw.npz')
u_pred2 = pred2['up'][:3]
...
c0 = ax[0, 0].contourf(x, y, u[0, :, :, n],   cmap=cmap,  levels = l, vmin = mi0, vmax = mx0)
c1 = ax[0, 1].contourf(x, y, u[1, :, :, n],   cmap=cmap,  levels = l, vmin = mi1, vmax = mx1)
c2 = ax[0, 2].contourf(x, y, u[2, :, :, n],   cmap=cmap,  levels = l, vmin = mi2, vmax = mx2)
c3 = ax[0, 3].contourf(x, y, omega_z[:, :, n],cmap=cmap,  levels = l, vmin = mi3, vmax = mx3)


ax[1, 0].contourf(x, y, u_pred1[0, :, :, n],    cmap = cmap, levels = l, vmin = mi0, vmax = mx0)
ax[1, 1].contourf(x, y, u_pred1[1, :, :, n],    cmap = cmap, levels = l, vmin = mi1, vmax = mx1)
ax[1, 2].contourf(x, y, u_pred1[2, :, :, n],    cmap = cmap, levels = l, vmin = mi2, vmax = mx2)
ax[1, 3].contourf(x, y, omega_z_pred1[:, :, n], cmap = cmap, levels = l, vmin = mi3, vmax = mx3)

ax[2, 0].contourf(x, y, u_pred2[0, :, :, n],    cmap = cmap,levels = l, vmin = mi0, vmax = mx0)
ax[2, 1].contourf(x, y, u_pred2[1, :, :, n],    cmap = cmap,levels = l, vmin = mi1, vmax = mx1)
ax[2, 2].contourf(x, y, u_pred2[2, :, :, n],    cmap = cmap,levels = l, vmin = mi2, vmax = mx2)
ax[2, 3].contourf(x, y, omega_z_pred2[:, :, n], cmap = cmap,levels = l, vmin = mi3, vmax = mx3)
