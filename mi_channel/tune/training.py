...
argparser.add_argument("--nn",default=100,type=int, help="Number of neurons")
argparser.add_argument("--nl",default=10,type=int, help="Number of layers")
...
case_name = f"pinn_t{n_time}_s{n_space}_c{c}_nn{args.nn}_nl{args.nl}_{args.sw}sw_{args.uw}uw"
...
for i in range(args.nl):
    hl = layers.Dense(args.nn, activation = act)(hl)
...
pinn = PINN(model, opt, n_training_with_adam, sw = args.sw ,uw= args.uw)
hist = pinn.fit(sp, cp)
...
np.savez_compressed(f'../results/res_' + case_name, up = up, hist = hist, comp_time = comp_time)
model.save(f'../results/' + case_name+'.h5')
